...
void Image_init(Image* img, std::istream& is) {
    if (img != nullptr) {
        std::string filemat;
        int maxintense;
        is >> filemat >> img->width >> img->height >> maxintense;
  ...
  if(img!=nullptr) {
    if(0 <= row && row < Image_height(img)) {
      if(0 <= column && column < Image_width(img)) {
        a.r=*Matrix_at(&(img->red_channel),row,column);
        a.g=*Matrix_at(&(img->green_channel),row,column);
        a.b=*Matrix_at(&(img->blue_channel),row,column);
      }
    }
  }
